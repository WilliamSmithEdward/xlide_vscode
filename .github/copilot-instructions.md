...
surface has 18 tools for discovering workbooks, reading/writing VBA, analyzing
...
**The workbook is the only source of truth for VBA. Exported files on disk are
not.**

Reach VBA one of two ways, both of which read and write the workbook itself:

- **XLIDE tools** - discover with `xlide_getWorkbookInfo` or
...
- **The XLIDE virtual file system** - modules opened from the XLIDE tree live at
  ...
  back into the workbook, so ordinary editor edits are workbook edits.

Both paths go through XLIDE's in-process workbook engine, which parses and
...
partial, and editing one changes nothing in the workbook. Treat them as source
