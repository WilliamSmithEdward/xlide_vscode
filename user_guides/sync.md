...
workbook's modules. XLIDE reads the live workbook modules, opens a preview, and
...
- switch export mode
- inspect workbook-vs-file diffs
...
module instead of the whole workbook.

## Export Modes

Export mode is workbook-specific:

| Mode | UI label | Behavior |
|---|---|---|
| `exportAll` | Export All (No Deletes) | Creates missing files and updates changed files for every workbook module. Stale files are left alone. |
| `trueUp` | Export All + Delete Missing | Does everything `exportAll` does, then previews stale root `.bas` and `.cls` module files for removal when they no longer exist in the workbook. |
...
- update existing workbook modules
...
workbook already contains a same-named module. XLIDE cannot create missing
...
Import mode is workbook-specific:

| Mode | UI label | Behavior |
|---|---|---|
| `updateOnly` | Import/Update (No Deletes) | Updates existing modules and creates supported missing standard/class modules. Workbook-only modules are left alone. |
| `trueUpStandardClass` | Import/Update + Delete Missing | Does everything `updateOnly` does, then previews workbook-only standard/class modules for deletion when they are missing from the folder. |

Document modules and UserForm code-behind modules are excluded from
`trueUpStandardClass` removals.

## Workbook Sync Settings

XLIDE stores workbook-specific sync settings beside the workbook in
...
unrelated settings and shows whether each value came from the workbook sidecar,
...
Import/export is preview-first. The workbook or file system is changed only
after you apply the preview.

Before a large import or any delete-enabled true-up, use source control or a
workbook backup so the change is easy to review or roll back.

Workbook imports are workbook mutations. If a workbook is signed, saving module
changes can invalidate the signature; XLIDE reports signature loss the same way
it does for other workbook write operations.

If a write fails because the workbook is locked by another application, close
the other copy and retry.

Workbook-to-workbook module transfer is not a v2 import/export workflow. Treat
...
AI agents should normally read and write workbook VBA through
...
- `xlide_exportModules` for exporting all workbook modules to a folder
- `xlide_configureExportMode` for setting the workbook-specific export mode

There is no v2 import agent tool. Import remains a user-previewed workflow so a
developer can inspect the workbook mutation before applying it.

After a meaningful import, run workbook analysis and any available workbook tests
before treating the workbook as release-ready.

## Troubleshooting

- If XLIDE cannot find a folder, choose one from the preview.
- If the sidecar is invalid, fix `<workbook>.xlide_settings.json` or remove the
  invalid fields and reopen the preview.
- If an import row says **Skipping import**, the file represents a document or
  UserForm code-behind module that does not already exist in the workbook.
- If a workbook write fails, check workbook protection, open-state conflicts,
