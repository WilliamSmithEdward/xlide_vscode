...
| `erase-requires-array` | An `Erase` target-list entry is a simple name that resolves to a known non-Variant scalar declaration such as `Dim obj As Object` or `Dim Value As Long`; arrays, scalar `Variant`, implicit Variant, unresolved names, non-simple member/index targets, and inactive branches stay quiet. Focused VBE oracle evidence rejects Object and Long scalar targets as `Expected array`, while scalar Variant compiles | Erase statement / VBE oracle: Expected array | src/analyzer/diagnostics/analyzeModule.ts | tests/vbaDiagnostics.test.ts + syntax_corpus/oracle/vbe_oracle_cases.json | Verified |
...
| `for-each-source-type` | A `For Each` source expression must be a collection object or array; parser-backed loop metadata records the `In` source expression and declaration-shape binding rejects simple source names declared as known intrinsic scalars. Arrays, `Variant`, `Object`, object-looking/unresolved sources, member chains, and inactive branches stay quiet. Focused VBE oracle evidence rejects the scalar source case as `For Each may only iterate over a collection object or an array` | 5.4.2.5 (For Each ... Next) / VBE oracle | src/analyzer/parser/parseModule.ts + src/analyzer/symbols/buildModuleSymbols.ts + src/analyzer/diagnostics/analyzeModule.ts | tests/vbaParser.test.ts + tests/vbaDiagnostics.test.ts + syntax_corpus/oracle/vbe_oracle_cases.json | Verified |
