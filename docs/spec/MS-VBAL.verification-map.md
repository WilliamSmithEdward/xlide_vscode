...
| `runtime-conversion-value` | A settled procedure statement calls bare or `VBA.`-qualified `CDate` with a plainly invalid string literal: empty string, or ASCII word text with no digits and no recognized English month token. Date-looking strings, non-ASCII/localized-looking text, variables, expressions, non-`CDate` conversion functions, source-shadowed bare `CDate` names, and inactive conditional-compilation branches stay quiet. Focused runtime oracle evidence confirms `CDate("not a date")` raises runtime error 13, while `CDate("1/2/2020")` runs under the oracle environment | 5.6 / runtime conversion to Date / VBE runtime error 13 | src/analyzer/diagnostics/analyzeModule.ts | tests/vbaDiagnostics.test.ts + syntax_corpus/oracle/vbe_oracle_cases.json | Verified |
