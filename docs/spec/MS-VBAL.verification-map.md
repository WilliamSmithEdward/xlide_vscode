...
| `unallocated-dynamic-array-access` | A local non-Static dynamic array declared as `Dim Values() As T` is indexed before any straight-line active `ReDim` allocation, or after `Erase Values`; module-level arrays, parameters, Static locals, fixed-size arrays, helper-call arguments, assignment-touched arrays, nested runtime-block touches that make state unknown, unresolved names, and inactive conditional-compilation branches stay quiet. Focused runtime oracle evidence confirms `Debug.Print Values(0)` before `ReDim` raises runtime error 9, `Subscript out of range` | VBA dynamic array allocation semantics / VBE runtime error 9 | src/analyzer/diagnostics/analyzeModule.ts | tests/vbaDiagnostics.test.ts + syntax_corpus/oracle/vbe_oracle_cases.json | Verified |
