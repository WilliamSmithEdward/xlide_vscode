...
| `member-not-found` | A source-backed workbook class receiver uses a member name absent from the class member surface; focused VBE oracle evidence rejects unknown property assignment and unknown method call as `Method or data member not found` while a known property control compiles | 5.6.9 / VBE oracle member binding | src/analyzer/diagnostics/analyzeModule.ts | tests/vbaDiagnostics.test.ts + syntax_corpus/oracle/vbe_oracle_cases.json | Verified |
