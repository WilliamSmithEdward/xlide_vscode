...
| Project visible type-name surface + module/type docs | src/analyzer/symbols/projectIndex.ts | tests/vbaSymbolGraph.test.ts | 5.2.3.1.4 / 5.2.3.3 / 5.2.3.4 / 4.2; docs are n/a (XLIDE convention) | Verified |
...
| `assignment-object-type-mismatch` | A `Set` assignment targets a known object variable or source-backed object-valued member, and the RHS is a deterministic incompatible object/scalar value; same project class and explicit `Implements` assignments are accepted | 5.4.3 / Set statement | src/analyzer/diagnostics/analyzeModule.ts + src/analyzer/symbols/projectIndex.ts | tests/vbaDiagnostics.test.ts + tests/vbaSymbolGraph.test.ts | Verified |
...
| `set-required` | A plain assignment targets a known object variable or source-backed object-valued member (`Property Set`/public field), where VBA requires `Set` | 5.4.3 / Set statement | src/analyzer/diagnostics/analyzeModule.ts | tests/vbaDiagnostics.test.ts | Verified |
