...
| `parameter-default-not-constant` | An Optional parameter default that is provably non-constant — a function/array call (`name(...)`), `New`, or `AddressOf` — which VBE rejects as `Constant expression required`. Bare and qualified identifiers (`Module.CONST`, `MyEnum.Value`) stay quiet (they may be constants), and object-typed parameters are deferred to `parameter-default-type-mismatch` so the two never double-report | 5.3.1.5 (Optional default-value is a constant-expression) | src/analyzer/diagnostics/rules/declarations.ts | tests/diagnostics/parameterDefaults.test.ts | Verified |
