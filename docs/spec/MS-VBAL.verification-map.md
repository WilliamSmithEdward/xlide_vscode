...
| `fixed-length-string-size` | A literal fixed-length `String` declaration size is outside the VBE-verified `1..65526` range; `String * 1` and `String * 65526` compile, while `String * 0` and `String * 65527` are rejected as `Invalid length for fixed-length string`. Nonliteral lengths are deferred until constant-expression semantics are verified | MS-VBAL fixed-length String bounds / VBE oracle | src/analyzer/diagnostics/analyzeModule.ts | tests/vbaDiagnostics.test.ts + syntax_corpus/oracle/vbe_oracle_cases.json | Verified |
