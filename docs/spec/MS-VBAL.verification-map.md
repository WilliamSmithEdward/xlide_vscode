...
| Member-access chain resolution | src/analyzer/completion/memberAccess.ts | tests/vbaMemberCompletion.test.ts | 5.6 (member access) plus host metadata return/default-member facts, active `With` receiver stacks, and simple `Set` assignment refinement for completion | Verified |
...
| `module-declaration-in-procedure` | Module-only forms inside procedure bodies: semantic coverage for completed statement forms such as `Option`, `Attribute`, `Def*`, and `Public`/`Private`/`Friend`/`Global` declarations; structural coverage for indented nested `Type`, `Enum`, `Declare`, and procedure declarations that would otherwise look block-balanced or break parser recovery | 5.2 (module declarations) / 5.3 (procedure declarations) | src/analyzer/diagnostics/analyzeModule.ts + src/vbaStructuralAnalysis.ts | tests/vbaDiagnostics.test.ts + tests/vbaStructuralAnalysis.test.ts + tests/vbaModuleAnalysis.test.ts | Verified |
...
| `assignment-object-type-mismatch` | A `Set` assignment targets a known object variable, Function/Property Get return name, or source-backed object-valued member, and the RHS is a deterministic incompatible object/scalar value, including scalar zero-argument Function/Property Get value references; same project class and explicit `Implements` assignments are accepted | 5.4.3 / Set statement | src/analyzer/diagnostics/analyzeModule.ts + src/analyzer/symbols/projectIndex.ts | tests/vbaDiagnostics.test.ts + tests/vbaSymbolGraph.test.ts | Verified |
