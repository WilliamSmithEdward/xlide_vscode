...
| `division-by-zero` | A settled procedure statement contains `/`, integer division `\`, or `Mod` with a numeric literal zero divisor; error severity because focused VBE oracle cases show these expressions compile but deterministically raise runtime error 11, while nonzero literal controls run | 5.6 / runtime division by zero | src/analyzer/diagnostics/analyzeModule.ts | tests/vbaDiagnostics.test.ts + syntax_corpus/oracle/vbe_oracle_cases.json | Verified |
