...
- Any casing normalizer must only alter tokens known to be keywords.
...
These compound forms must be emitted correctly when generated by snippets,
fixes, or casing normalizers:
...
## Phase 8: Keyword Capitalization

### Goal

Keep active keyword casing canonical through completion and edit-boundary
normalization without creating a separate formatter or on-save pipeline.

### Features

- Completion and snippets insert canonical keyword spelling.
- Edit-boundary formatting normalizes only real keyword tokens on the active
  line after edits settle.
- Do not add a separate document-wide code action or on-save setting for keyword
  casing unless it is explicitly justified later as a distinct workflow.
...
- Safe on ugly mixed-case lines.
- Does not change comments or strings.
- Does not change identifiers.
- Uses the same casing helper as completion and edit-boundary normalization.
...
```json
{
  ...
  "xlide.docs.enabled": true,
  "xlide.docs.metadataGlob": "**/*.vbref.xml"
}
```
...
- Diagnostics: none.
- Active keyword casing normalization: canonical keyword spellings without
  touching comments, strings, or identifiers.
...
6. Safe keyword capitalization normalization.
