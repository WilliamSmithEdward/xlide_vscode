...
| Division by zero | Verified for current slice | Runtime oracle cases for `/ 0`, `\ 0`, `Mod 0`, `/ 2`, hex/octal zero, nonzero hex, module/local `Const` zero, zero/nonzero `Const` expressions; analyzer diagnostics for these zero divisors in settled procedure statements, including nested expressions | Flow/reachability, `On Error Resume Next`, hex/octal `Const` folding, enum constants, and broader expression-value folding | Runtime oracle for each operator family plus conservative unit tests |
