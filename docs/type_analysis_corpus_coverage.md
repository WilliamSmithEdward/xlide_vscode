...
| Classes and document modules | Partial | `ProjectIndex.visibleTypeNames()`, `ProjectIndex.projectClassMembers()`, member-completion tests for `Dim p As Person: p.` and `Me.`, source-backed object member go-to-definition including `Me.Member`, inline-doc completion/hover tests for `p.Age`, source-backed class member signature help with inline summary/parameter docs, source-backed member-call arity/type diagnostics, property and public-field assignment diagnostics for typed writable members and read-only properties, `member-not-found` diagnostics for source-backed class receivers, oracle coverage for public fields as writable object members, `Public Const` exclusion from object-member surfaces, module-kind-sensitive public object-module declaration diagnostics, late-bound `Object`/`Variant` unknown-member compile controls and no-diagnostic tests even after `Set` assignment, exported `VB_UserMemId = 0` default-member attribute extraction with source spans, semantic-token tests for `As Person` and `New Person` | Class-level/module-level docs, class instances beyond current member-assignment slice, default-member expression semantics such as `textValue = p`, document-module event handler authoring scoped to workbook/worksheet/chart/UserForm modules, declared `Event` members, `WithEvents` bindings, Friend visibility, `Property Set` object assignment, document/UserForm designer members, broader member-call diagnostics | Project fixture builder and binder tests; oracle only when VBA behavior affects diagnostics |
| Excel object model receiver chains | Partial | Legacy host-pattern corpus, oracle `thisworkbook_unknown_member_compile`, generated member surfaces for `Application`/`Workbook`/`Worksheet`/`Range`/`Workbooks`/`Worksheets`/`Sheets`, generated host member signatures/docs in completion, hover, signature help, and member-call arity/type diagnostics, exhaustive host diagnostics for `Excel.Workbook` and `Excel.Worksheet`, tests for `ThisWorkbook.doesnotexist`, `ActiveSheet.asdf`, declared `Worksheet` missing members, `Worksheets(index).Range`, merged worksheet/chart `Sheets(index).Range`, completion-only generic `Object`/`Variant` refinement from simple `Set` assignments, and generated non-exhaustive members, plus `docs/excel_reference_coverage.md` | Broader Excel object surfaces, property-vs-method call semantics beyond collection-default `Item`, consistent object-access diagnostics across host/source/external metadata, promotion of additional dump-backed host types to exhaustive only after controls | Generated/reference dump metadata plus integration tests; hard diagnostics only after exhaustive provenance or oracle-backed controls |
...
9. **Document-module event handler matrix**: curate workbook, worksheet, chart,
   and UserForm event signatures; test completion/insertions, exact casing,
   module-type scoping, and non-red wrong-module guidance.
...
12. **Reference dump coverage matrix**: extend the generated
    ...
    imported, which gaps remain, and whether the type is eligible for
