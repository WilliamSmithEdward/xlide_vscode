...
| Runtime argument value bounds | Verified for current slice | Runtime oracle cases for negative and zero `Left`/`Left$` Length and `String`/`String$` Number arguments; analyzer diagnostics for negative integer literals in settled procedure statements with bare-call shadowing respected | Broader runtime-function bounds such as `Space`, `Mid`, `Right`, `Replace`, nonliteral constant folding, and `On Error Resume Next`/reachability policy | Runtime oracle for each function family plus conservative unit tests |
