...
| Runtime argument value bounds | Verified for current slice | Runtime oracle cases for negative and zero `Left`/`Left$`, `Right`/`Right$`, `String`/`String$`, and `Space`/`Space$` bounds plus `Mid`/`Mid$` Start/Length and `Replace` Start/Count bounds; analyzer diagnostics for out-of-bound integer literals, reducible integer expressions, same-module/procedure `Const` values, current-module Enum members, and bare/module-qualified visible exported standard-module `Const`/Enum members in settled procedure statements with bare-call and named-argument handling | Broader runtime-function bounds, additional named-argument edge cases, hidden-helper constant dependencies, and `On Error Resume Next`/reachability policy | Runtime oracle for each function family plus conservative unit tests |
| Division by zero | Verified for current slice | Runtime oracle cases for `/ 0`, `\ 0`, `Mod 0`, `/ 2`, hex/octal zero, nonzero hex, module/local decimal/hex/octal `Const` zero, zero/nonzero `Const` expressions, same-module Enum member values, and bare/module-qualified visible exported standard-module `Const`/Enum values; analyzer diagnostics for these zero divisors in settled procedure statements, including nested expressions | Flow/reachability, `On Error Resume Next`, hidden-helper constant dependencies, and broader expression-value folding | Runtime oracle for each operator family plus conservative unit tests |
