...
| Runtime argument value bounds | Verified for current slice | Runtime oracle cases for negative and zero `Left`/`Left$`, `Right`/`Right$`, `String`/`String$`, and `Space`/`Space$` bounds plus `Mid`/`Mid$` Start/Length bounds; analyzer diagnostics for out-of-bound integer literals in settled procedure statements with bare-call shadowing respected | Broader runtime-function bounds such as `Replace`, nonliteral constant folding, named-argument edge cases, and `On Error Resume Next`/reachability policy | Runtime oracle for each function family plus conservative unit tests |
...
| String family | Partial | Fixed-length declaration parser/symbol/hover tests; oracle-backed literal bounds for `String * 1`, `String * 65526`, `String * 0`, and `String * 65527`; deterministic fixed-length `Const` expression diagnostics; runtime argument value diagnostics for `Left`/`Left$`, `Right`/`Right$`, `String`/`String$`, `Space`/`Space$`, and `Mid`/`Mid$`; fixed-length string and string-size legacy corpus cases | Fixed-length assignment truncation, non-deterministic length expression semantics, broad type-suffix call normalization | Spec and oracle mix |
