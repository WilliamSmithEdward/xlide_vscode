...
| Division by zero | Verified for literal-zero slice | Runtime oracle cases for `/ 0`, `\ 0`, `Mod 0`, and `/ 2` valid control; analyzer diagnostics for numeric literal zero divisors in settled procedure statements, including nested expressions | Named constants, constant expressions, hex/octal zero, flow/reachability, `On Error Resume Next`, and broader expression-value folding | Runtime oracle for each operator family plus conservative unit tests |
