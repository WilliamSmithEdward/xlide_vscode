...
| Const / Enum constant expressions | Partial (non-constant shape shipped) | `const-value-not-constant` and `enum-member-not-constant` flag a `Const` value or `Enum` member initializer that is provably non-constant — a call (`name(...)`), `New`, or `AddressOf` (VBE "Constant expression required"). Module-level and procedure-local (incl. nested-block) `Const`s covered; implicit enum members skipped; literals/concat/arithmetic/grouping and bare/qualified identifiers stay quiet. Shares the operator-keyword-aware `nonConstantDefaultElement` gate with the parameter-default rule; adversarial FP/FN hunt verified empirically (`tests/diagnostics/constEnumNotConstant.test.ts`) | Non-integer constant folding (string/float arithmetic, type coercion) needs a constant-expression binder; out-of-range/overflow values need the runtime oracle; `#If` inside an Enum body is conservatively skipped pending a parser fix | Binder for full constant folding; oracle for overflow |
