...
| Scalar assignment coercion | Partial | Runtime oracle matrix for numeric, Boolean, String controls; Byte/Integer positive decimal literal boundary controls now prove in-range values run and out-of-range values raise Overflow | Date, Currency edge formats, Decimal, broader Byte/Integer negative/suffix/hex bounds, Long/LongLong/LongPtr overflow vs type mismatch | Focused runtime oracle cases |
...
| Broad numeric family | Partial | Some `Integer`, `Double`, `Currency`, `Long` examples; oracle-backed Byte/Integer positive decimal literal overflow for assignments and ByVal arguments | Long/LongLong/LongPtr, Single, Decimal, negative bounds, suffixes, hex/octal literals, fixed-width overflow beyond Byte/Integer | Runtime oracle for coercion/overflow; parser tests for suffixes |
