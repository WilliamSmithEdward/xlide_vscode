...
| Runtime argument value bounds | Verified for current slice | Runtime oracle cases for negative and zero `Left`/`Left$`, `Right`/`Right$`, `String`/`String$`, and `Space`/`Space$` bounds plus `Mid`/`Mid$` Start/Length and `Replace` Start/Count bounds; analyzer diagnostics for out-of-bound integer literals in settled procedure statements with bare-call and named-argument handling | Broader runtime-function bounds, nonliteral constant folding, additional named-argument edge cases, and `On Error Resume Next`/reachability policy | Runtime oracle for each function family plus conservative unit tests |
