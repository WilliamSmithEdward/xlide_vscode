...
| Scalar assignment coercion | Partial | Runtime oracle matrix for numeric, Boolean, String controls; Byte/Integer/Long decimal literal boundary controls now prove in-range min/max values run and out-of-range values raise Overflow (Long verified via `long_assignment_*_literal_runtime`: ±2^31 controls run, 2147483648 / -2147483649 raise Run-time error '6'). No-diagnostic boundary (verified): a non-literal `String` -> numeric assignment (e.g. `n = s`, or a `String`-returning member/function) intentionally stays quiet because VBA coerces String to numeric at runtime; only a provably-non-numeric string **literal** (`n = "hello"`) flags. Flagging the runtime-convertible case would be a false positive | Date, Currency edge formats, Decimal, Byte/Integer/Long suffix/hex bounds, LongLong/LongPtr overflow vs type mismatch | Focused runtime oracle cases |
| Scalar argument coercion | Partial | Currency parameter, native `Left` length, numeric-string controls, Byte/Integer/Long decimal literal boundary controls for ByVal arguments | Boolean arguments, Date arguments, more native signatures, ByRef arguments, suffix/hex/octal numeric literal bounds beyond current decimal slice | Runtime oracle and curated metadata tests |
...
| Broad numeric family | Partial | Some `Integer`, `Double`, `Currency`, `Long` examples; oracle-backed Byte/Integer/Long decimal literal overflow for assignments and ByVal arguments. Long is the safe-integer ceiling: only bare decimal literals within JS safe-integer range carry a `numericValue`, so hex/octal/suffixed/float literals never range-check, and LongLong/LongPtr are deliberately omitted (any safe-integer literal already fits ±2^63, and LongPtr width is platform-dependent) | LongLong/LongPtr, Single, Decimal, suffixes, hex/octal literals, fixed-width overflow beyond Byte/Integer/Long | Runtime oracle for coercion/overflow; parser tests for suffixes |
...
| UDTs | Partial | Parser/symbol graph declarations, visible cross-module UDT names, source-backed UDT member completion/diagnostics including leading-dot `With` fields, and workbook-analysis project-type canary coverage for valid field access plus hard `member-not-found` on an unambiguous missing UDT field | Object members in UDTs, ambiguous member-name behavior, nested UDT fields, arrays in UDTs, module-qualified UDT type-name edge cases | Spec plus compile oracle for edge cases |
