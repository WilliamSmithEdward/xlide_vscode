...
1. **Workbook fixture files**: initial workbook-analysis assertions now run from
   ...
   declarations, parser-backed `WithEvents` declaration diagnostics,
   token-backed `Implements` placement diagnostics, same-module `RaiseEvent`
   target diagnostics, same-workbook live-source overlays, cross-workbook
