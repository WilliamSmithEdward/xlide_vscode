...
Purpose: keep XLIDE responsive while editing real VBA projects, and make slow
...
| Project-wide analysis command | 5 s p95 for ordinary projects | 10 s |
| Project index rebuild after one module save | 500 ms p95 | 2 s |
| Project tree refresh for ordinary workspace folders | 1 s p95 | 5 s |
...
- Never run `analyzeVbaModuleSource` on the extension host on a keystroke-paced
  ...
  only what changed.
- When the worker is down, large project modules drop the local pass entirely
...
- Analyze Project rides the same worker: each module's analysis runs
...
## Project Engine Budgets

Every project operation runs in-process against the file on disk, so these are
whole-call costs with no IPC or process startup in them. Measured on a 2.4 MB
project with 42 modules (621 KB of VBA source), which is past the "medium"
fixture below:

| Operation | Target | Measured p50 |
|---|---:|---:|
| `listModules`, `listSubs`, `getProtectionInfo`, `validateProject` | 25 ms | ~4 ms |
...
| `createProject` | 100 ms | <1 ms |

Warm calls are two orders of magnitude cheaper again: reads against a project
...
exactly one parse. On a 42-module project that is 44 calls for ~22 ms total.
...
- **Reads share one parse per project.** `projectService` caches the parsed
  ...
  path (`openContainerForWrite`) - handing a writer the shared parse poisons it
...
- Small project: 1-10 modules, under 5,000 total VBA lines.
- Medium project: 10-50 modules, under 50,000 total VBA lines.
- Large project: 50-250 modules, under 250,000 total VBA lines.
- Large single module: 10,000 lines.
- Diagnostic-heavy module: at least 500 findings without UI lockup.

If a project exceeds these assumptions, XLIDE should still avoid blocking the
...
- Use `XLIDE: Copy Performance Snapshot` after reproducing latency in the
  ...
  help, semantic tokens, live diagnostics, project context indexing, tree
  expansion, commands, project engine calls, virtual file reads/writes, project
...
- Preserve correctness before speed. Performance work must not silently skip
  diagnostics, project symbols, or project modules.

## Required Hardening Before Broad Release

- Add at least one large-project fixture or synthetic project generator that
...
- Invalidate caches by source text, project identity, module identity, settings,
...
- `tree.getChildren`
- `projectContext.getAllModules`
- `analyzeProject.readModules`, `analyzeProject.listModules`,
  `analyzeProject.readModule`
- `analyzeProject.buildProjectContext`, `analyzeProject.settings`,
  `analyzeProject.analyzeModule`, `analyzeProject.total`
- `command`
- `filesystem.readFile`, `filesystem.writeFile`
- `docs.reload`
- `sidebar.render`, `sidebar.projectFiles`
- `moduleExport.single`, `moduleExport.project`
- `moduleSync.buildExportPlan`, `moduleSync.buildImportPlan`,
  `moduleSync.renderHtml`, `moduleSync.refreshFromDisk`,
  `moduleSync.refreshProjectSettings`, `moduleSync.refreshSettings`,
  `moduleSync.chooseFolder`, `moduleSync.apply`
- `projectAnalysis.renderPanel`
