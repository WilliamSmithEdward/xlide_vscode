...
import {
    officeComAvailabilityFromProbe,
    officeComProbePowerShellScript,
} from '../src/officeComAvailability';

describe('Excel COM availability', () => {
    it('checks COM registration without creating an Excel instance', () => {
        const script = officeComProbePowerShellScript();
    ...
    it('reports ready when the Excel COM ProgID is registered', () => {
        expect(officeComAvailabilityFromProbe('win32', 0, 'XLIDE_OFFICE_COM_OK', '')).toEqual({
            state: 'installed',
    ...
    it('reports missing when the Excel COM ProgID is absent', () => {
        expect(officeComAvailabilityFromProbe('win32', 2, 'XLIDE_OFFICE_COM_MISSING', '')).toEqual({
            state: 'missing',
    ...
    it('blocks non-Windows test execution before probing Excel', () => {
        expect(officeComAvailabilityFromProbe('linux', null, '', '')).toEqual({
            state: 'blocked',
...
describe('the probe follows the file host (issue #24)', () => {
    it('probes the ProgID of the requested application', () => {
        expect(officeComProbePowerShellScript('word')).toContain('Word.Application');
        expect(officeComProbePowerShellScript('powerpoint')).toContain('PowerPoint.Application');
        expect(officeComProbePowerShellScript()).toContain('Excel.Application');
    });

    it('names the requested application in every outcome', () => {
        const missing = officeComAvailabilityFromProbe('win32', 2, 'XLIDE_OFFICE_COM_MISSING', '', 'word');
        expect(missing.title).toBe('Word COM Not Found');
        expect(missing.description).toContain('Microsoft Word');
        const ready = officeComAvailabilityFromProbe('win32', 0, 'XLIDE_OFFICE_COM_OK', '', 'powerpoint');
