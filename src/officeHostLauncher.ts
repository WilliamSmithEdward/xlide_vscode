...
/** Leaves the running (or a new) Access in `$app` with the target database open. */
...
export function buildHostOpenScript(options: HostOpenScriptOptions): string {
    const { host, filePath, attachToRunning, readOnly } = options;
    if (host === 'excel') {
        return buildExcelLaunchScript({ filePath, attachToRunning, mode: { kind: 'open', readOnly } });
    }
    const target = [
        '$ErrorActionPreference = "Stop"',
        COM_RETRY_HELPER,
    ...
    if (host === 'word') {
        return [
            ...
            ...foregroundLines('$app.ActiveWindow.Hwnd', '$file.Activate()'),
        ].join('; ');
    }
    if (host === 'powerpoint') {
        return [
            ...
            ...foregroundLines('$app.HWND', '$file.Windows.Item(1).Activate()'),
        ].join('; ');
    }
    return [
        ...target,
...
export async function openFileInHost(
    ...
    log(`[openInHost] Opening in ${appName}: powershell -Command "${script}"`);
